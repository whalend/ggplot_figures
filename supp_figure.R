...
# Full process I went through to develop the figure ####
d1 <- read.xlsx("data/SOCO_plot-level_covariates_n163_for_Nathan.xlsx", 2)
str(d1)
d1 <- na.omit(d1)

bay_dens <- d1 %>% select(c(plotid:umca_ct_11))
saveRDS(bay_dens, file = "bay_dens.rds")
bay_dens <- bay_dens %>% gather(year, bay_dens, umca_ct_04:umca_ct_11) %>% mutate(year = extract_numeric(year), year = as.factor(year+2000))

bay_slc <- d1 %>% select(c(plotid,cum_slc_04:cum_slc_11))
saveRDS(bay_slc, "bay_slc.rds")
bay_slc <- bay_slc %>% gather(year, slc, cum_slc_04:cum_slc_11) %>% mutate(year = extract_numeric(year), year = as.factor(year+2000))


bay_vars1 <- inner_join(bay_dens,bay_slc, by = c("plotid","year"))
str(bay_vars1)
head(bay_vars1)
saveRDS(bay_vars1, file = "data/bay_variables.rds")

bay_vars1 <- readRDS("data/bay_variables.rds")
head(bay_vars1)
tail(bay_vars1)
summary(bay_vars1)
str(bay_vars1)
